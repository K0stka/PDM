import { archetypes as Archetypes, interests as Interests, count, db, eq, events, getTableColumns } from "@/db";
	...
	return (
		<PageTemplate title="Anotace přednášek">
			<div className="flex flex-col gap-5">
				{interestsActive() && user.isAttending && <Card className="p-5 text-muted-foreground">{configuration.interestsCTA}</Card>}
				{archetypes.map((archetype) => (
					<SharedArchetypeElement
						key={archetype.id}
						archetype={archetype}
						canExpressInterest={canUserExpressInterest(user, interests.length)}
						isInterested={interests.some((interest) => interest.archetype === archetype.id)}
					/>
				))}
			</div>
		</PageTemplate>
