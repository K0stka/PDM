import { Edit, Filter, FilterX, GraduationCap, Presentation, RefreshCcw, Shield, SquareCheck, SquareDashed, UserIcon, XSquare } from "lucide-react";
...
export const getColumns: (options: ColumnsProps) => ColumnDef<User>[] = ({ filter, roleFilterActive, cycleRoleFilter, cancelRoleFilter, onEditUser }) => [
	{
		accessorKey: "name",
		header: ({ column }) => (
		...
		header: () => (
			<Popover>
				...
				<PopoverContent
					...
					{(["attending", "teacher", "presenting", "admin"] as (keyof typeof roleNames)[]).map((role) => (
						<Button
							variant="ghost"
							key={role}
							onClick={() => cycleRoleFilter(role)}
							className="h-auto p-2 text-sm cursor-pointer col-span-2 grid-cols-subgrid grid">
							<span className="flex items-center gap-2">
								{getRoleIcon(role)} {getRoleName(role)}
							</span>
							{filter[role] === null ? <SquareDashed /> : filter[role] ? <SquareCheck /> : <XSquare />}
						</Button>
					))}
					{roleFilterActive && (
						<Button
							variant="ghost"
							className="h-auto p-2 text-sm cursor-pointer text-muted-foreground col-span-2 grid-cols-subgrid grid"
